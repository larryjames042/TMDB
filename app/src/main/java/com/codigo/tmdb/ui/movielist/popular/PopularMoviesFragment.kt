package com.codigo.tmdb.ui.movielist.popular
...
import com.codigo.tmdb.viewmodel.PopularMoviesFragmentViewModel
import dagger.hilt.android.AndroidEntryPoint
import timber.log.Timber
import com.codigo.tmdb.ui.MainActivity
...
class PopularMoviesFragment : Fragment() {
    ...
    private val movieViewModelPopular : PopularMoviesFragmentViewModel by viewModels()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        movieViewModelPopular.movies.observe(viewLifecycleOwner, Observer {
            Timber.tag("dataresponse").d(it.toString())
            it?.apply {
         ...
         */
        movieViewModelPopular.networkResponseState.observe(viewLifecycleOwner, Observer {
            // hide progress bar
         ...
         */
        movieViewModelPopular.favInsertResult.observe(viewLifecycleOwner, Observer {
            if(it.toInt()== -1){
                // already exist in database
                Toast.makeText(activity, "Removed from favourite", Toast.LENGTH_SHORT).show()
                mMovie?.run {
                    movieViewModelPopular.removeFavouriteData(mMovie!!)
                }
            }else{
                // successfully added
    ...
    private fun setUpRecyclerView(){
        movieListAdapter = MoviesListAdapter(MovieItemClick { img, movie->
            ...
            movieViewModelPopular.insertFavouriteData(movie)
        })
